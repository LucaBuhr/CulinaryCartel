...
// Get the userID from the database for the currentPageUsername
$query = "SELECT userID FROM Users WHERE Username = '$currentPageUsername'";
$result = $conn->query($query);

if ($result->num_rows == 1) {
    $row = $result->fetch_assoc();
    $currentPageUserID = $row['userID'];

    // Compare with logged-in user's userID
    if ($_SESSION['userID'] != $currentPageUserID) {
        header("Location: ../unauthorized.php"); // Redirect to unauthorized or login page
        exit;
    }
} else {
